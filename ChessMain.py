...
WIDTH = HEIGHT = 512
DIMENSION = 8 #dimensions of chess board is 8*8
SQ_SIZE = HEIGHT // DIMENSION
MAX_FPS = 15 #mostly for animations later on
IMAGES = {}
...
def loadImages():
    ...
    #Note : we can access image by using piece name as key

'''
THIS is our main driver it will handle the user input and updating the graphics
'''
def main():
    p.init()
    ...
    animate = False #flag variable when we should animate
    loadImages() #only do this once before the while loop
    running = True
    sqSelected = () #no of square is selected , keep track of the last click of the user (tuple)
    ...
    while running:
        ...
        for e in p.event.get():
            ...
            # key handlers
                ...
                if e.key == p.K_z: #undoe when 'z' is pressed
                    gs.undoMove()
                    moveMade = True
                    animate = False
                    gameOver = False
...
def hightlightSquares(screen, gs, validMoves, sqSelected):
    if sqSelected != ():
        ...
        if gs.board[r][c][0] == ('w' if gs.whiteToMove else 'b'): #sqSelected is a piece that can be moved
            ...
            #hightligt move from that square
    ...
    for frame in range(frameCount + 1):
        ...
        if move.pieceCaptured != "--":
            if move.isEnpassantMove:
                enPRow = (move.endRow + 1) if move.pieceCaptured[0] == "b"  else (move.endRow - 1)
                endSquare = p.Rect(move.endCol * SQ_SIZE, enPRow * SQ_SIZE, SQ_SIZE, SQ_SIZE)
